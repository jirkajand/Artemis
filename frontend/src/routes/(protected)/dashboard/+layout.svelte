<script lang="ts">
	...
	const dashboardPages: DashboardPage[] = [
		...
		{ label: 'My buddies', path: '/dashboard/my-students' }
	];
	...
	$effect(() => {
		...
		if (currentPath !== lastPath) {
			...
			if (match && match.path !== selected.path) selected = match;
...
<TopAppBar variant="static">
	<Row>
		...
		<Section class="segment-section">
			<SegmentedButton segments={dashboardPages} singleSelect bind:selected key={s => s.path}>
