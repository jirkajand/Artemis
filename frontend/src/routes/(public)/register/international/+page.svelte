<script lang="ts">
  ...
  import Tab, { Label } from "@smui/tab";
	...
	import CountryCodeInput from "$lib/components/registration/CountryCodeInput.svelte";
  ...
  const formInitial: Omit<RegisterInternationalStudentRequest, 'dateOfBirth'> & {
    dateOfBirth: string
  } = {
    firstName: '',
    lastName: '',
...
<h1>Register - International Student</h1>
...
<form onsubmit={submit}>
  ...
  <section>
    <PhoneNumberInput bind:value={form.phoneNumber} label="Phone Number" />
    <Select bind:value={form.gender} label="Gender" required>
      <Option value={GenderEnum.Male}>Male</Option>
      <Option value={GenderEnum.Female}>Female</Option>
      <Option value={GenderEnum.Other}>Other</Option>
    </Select>
    <Select bind:value={form.facultyId} label="Faculty">
  ...
  <section>
    <CountryCodeInput combobox bind:value={form.countryISO} style="width: 100%;" label="Country of Origin" />
  </section>
